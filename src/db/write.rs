...
impl WriteDb {
    ...
    /// Insert multiple spans in a single transaction
    ...
    /// Look up session_ids for trace_ids (for spans arriving in different batches)
    ...
    /// Get spans older than the hot window (by created_at)
    ...
    /// Delete spans older than the hot window
    ...
    /// Get the internal connection (for testing)
    #[cfg(test)]
